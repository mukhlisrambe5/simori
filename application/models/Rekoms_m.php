...
class Rekoms_m extends CI_Model {
     ...
     private function _get_datatables_query() {
         ...
         $i = 0;
         foreach ($this->column_search as $rekoms) { // loop column 
             if(@$_POST['search']['value']) { // if datatable send POST for search
                 if($i===0) { // first loop
                     $this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
                     $this->db->like($rekoms, $_POST['search']['value']);
                 } else {
                     $this->db->or_like($rekoms, $_POST['search']['value']);
                 }
    ...
    public function get_wait($id=null){
        ...
        $this->db->join('tb_status', 'tb_status.status_id = tb_rekom.status_id');
        $this->db->join('tb_user', 'tb_user.user_id = tb_rekom.user_id');
        $this->db->where('tb_rekom.status_id', '2');
        $this->db->where('comment !=', '');
        $this->db->or_where('file !=', '');
